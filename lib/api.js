    ...
    createCommit(options) {
        return new Promise((resolve, reject) => {
            ...
            let filePath = path.join(
                ...
                (options.date.getMonth() + 1).toString(),
                options.date.getDate().toString()
            );
            ...
            mkdirp(path.dirname(filePath), (err) => {
                ...
                fs.writeFile(filePath, Math.random().toString(), (err) => {
                    if (err) {
                        return reject(err);
                    }

                    git.add(options.directory, '.').then((added) => {
                        return git.commit(options.directory, 'This is a commit.');
                    }).then((committed) => {
